...
import com.health.pusun.device.views.AutoFitTextureView;
...
public class Camera2VideoFragment extends Fragment
    ...
    private static Size chooseVideoSize(Size[] choices) {
        for (Size size : choices) {
            if (size.getWidth() == size.getHeight() * 16 / 9 && size.getWidth() <= 1380) {
//                if (size.getWidth() == size.getHeight() * 16 / 9 && size.getHeight() == 1080) {
    ...
    @Override
    public void onPause() {
//        if(MainActivity.isRecording == 1){stopRecordingVideo();}
        closeCamera();
    ...
    public static boolean isCameraCanUse() {
        boolean canUse = false;        Camera mCamera = null;
        try {
    ...
    private String getVideoFilePath(Context context) {
        ...
        if ("null".equals(smdtManager.smdtGetUSBPath(context, 0))) {
            ShowHelper.toastShort(getActivity(), "未插入U盘");
            return null;
        }
        File mediaStorageDir = new File(
                smdtManager.smdtGetUSBPath(context, 0) + File.separator
                        + "/Video/");
...
//        filePath = mediaFile.getAbsolutePath();
    ...
    private void stopRecordingVideo() {
        // UI
        try {
            ShowHelper.toastShort(getActivity(), "停止录像");
    ...
    @Subcriber
    private void dealStartRecordEvent(CameraTwoStartMsg startRecordMsg) {

        if(startRecordMsg.code == 1){
            ShowHelper.toastShort(getActivity(),"开始录像");
            startRecordingVideo();
        }
